defmodule Assemblage.Multimedia do
  @moduledoc """
  The Multimedia context. Effectively allows CRUD capability
  for a proposed multimedia library.

  The multimedia types are all associated with users.
  NOTE that this association is one-way: the users do not own the
  images in a meaningful sense, the images just have a `user_id`
  field that corresponds to the user in question's `id`.
  """
  ...
  @doc """
  Returns the list of all images, preloading the user info
  for each.
  ...
  @doc """
  Given a user + required attributes, creates a image.
  ...
  """
  def update_image(%User{} = user, %Image{} = image, attrs) do
    image
    |> Image.changeset(attrs)
    |> put_user(user)
    |> Repo.update()
